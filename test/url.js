...
describe('Url()', function () {
    ...
    it('should match current dir when constructed with no argument', function () {
      const u = new Url();
      const dir = u.path.replace(/\//g, p.sep);
      process.platform.match(/^win/) && (dir = dir.substr(1));
      assert.equal(dir, fs.realpathSync('.'));
    ...
    it('should correctly handle URLs with special characters', function () {
      const originalUrl = 'http://example.com%2Fpath%3Fparam%3D%24value%5E%26test%3D@example.com/path?param=%24value%5E&test=%40test';
      const u = new Url(originalUrl);
      assert.equal(u.toString(), originalUrl);
    });
  
 
  
  
    it('should handle URLs with array query parameters', function () {
      const originalUrl = 'http://localhost/?a&a&a';
      const u = new Url(originalUrl);
      assert.equal(u.query.toString(), 'a&a&a');
      assert(Array.isArray(u.query.a));
      assert.equal(u.query.a[0], null);
      assert.equal(u.query.a[1], null);
      assert.equal(u.query.a[2], null);
      assert.equal(u.queryLength(), 1);
  ...
  describe('Path URL encoding', function () {
    ...
    it('should correctly encode Plus Sign (+) to %2b in path.', function () {
      const u = new Url('http://localhost/path+with+plus');
      assert.equal(u.toString(), 'http://localhost/path%20with%20plus');
    });
  
    it('should preserve Plus Sign (+) in path.', function () {
      const u = new Url('http://localhost/path+with+plus', false);
      assert.equal(u.toString(), 'http://localhost/path%20with%20plus');
    });
  });